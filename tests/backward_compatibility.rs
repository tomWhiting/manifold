use manifold::{Legacy, ReadableDatabase, ReadableTableMetadata, TableError};
...
fn tuple_types() {
    ...
    let db = manifold::Database::open(tmpfile.path()).unwrap();
    let read_txn = db.begin_read().unwrap();
    let bad_table_def: manifold::TableDefinition<(u64, &str), &str> =
        manifold::TableDefinition::new("table");
    assert!(matches!(
        read_txn.open_table(bad_table_def).unwrap_err(),
        TableError::TableTypeMismatch { .. }
    ));
    let table_def: manifold::TableDefinition<Legacy<(u64, &str)>, &str> =
        manifold::TableDefinition::new("table");
    let table = read_txn.open_table(table_def).unwrap();
    assert_eq!(table.len().unwrap(), ELEMENTS as u64);
    for i in 0..ELEMENTS {
        ...
        let value = result.value();
        let bytes = <&str as manifold::Value>::as_bytes(&value);
        let expected = &<&str>::make_data()[i];
        let expected_bytes = <&str as manifold::Value>::as_bytes(expected);
