...
fn main() {
    ...
    println!("Starting concurrent writes with Durability::None...");
        ...
        let handle = std::thread::spawn(move || {
            ...
            for j in 0..1000 {
                ...
                txn.set_durability(Durability::None).unwrap();
                {
                    ...
                    table
                        .insert(&((i * 1000 + j) as u64), data.as_slice())
                        .unwrap();
                }
                txn.commit().unwrap();
            }
    ...
    println!(
        "\n4 CFs × 1000 writes = 4000 total writes in {:?}",
        duration
    );
    println!("Throughput: {:.0} ops/sec", 4000.0 / duration.as_secs_f64());
    println!(
        "Per-thread throughput: {:.0} ops/sec",
