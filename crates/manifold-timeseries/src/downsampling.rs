...
impl<'txn, E: TimestampEncoding> TimeSeriesTable<'txn, E> {
    ...
    pub fn downsample_minute_to_hour(
        ...
        self.downsample_aggregates(series_id, start_ms, end_ms, Granularity::Minute, Granularity::Hour)
    }
    ...
    pub fn downsample_hour_to_day(
        ...
        self.downsample_aggregates(series_id, start_ms, end_ms, Granularity::Hour, Granularity::Day)
    }
...
mod tests {
    ...
    fn test_downsample_to_minute() {
        ...
        // Verify minute aggregates
        {
            ...
            let ts_read = crate::timeseries::TimeSeriesTableRead::<AbsoluteEncoding>::open(&read_txn, "cpu").unwrap();

            // Minute 1 aggregate (rounded to 60000)
            let agg1 = ts_read.get_aggregate(Granularity::Minute, "server1", 60_500).unwrap().unwrap();
            ...
            let agg2 = ts_read.get_aggregate(Granularity::Minute, "server1", 120_500).unwrap().unwrap();
    ...
    fn test_downsample_minute_to_hour() {
        ...
        // Manually write minute aggregates
        {
            ...
            ts.minute_table_mut().insert((60_000, "server1"), &Aggregate {
                min: 10.0,
                max: 20.0,
                sum: 100.0,
                count: 10,
            ...
            ts.minute_table_mut().insert((120_000, "server1"), &Aggregate {
                min: 15.0,
                max: 25.0,
                sum: 150.0,
                count: 15,
        ...
        // Downsample to hour
        {
            ...
            let count = ts.downsample_minute_to_hour("server1", 0, 3_600_000).unwrap();
        ...
        // Verify hour aggregate
        {
            ...
            let agg = ts_read.get_aggregate(Granularity::Hour, "server1", 120_000).unwrap().unwrap();
            assert_eq!(agg.min, 10.0); // min of mins
            assert_eq!(agg.max, 25.0); // max of maxes
            assert_eq!(agg.sum, 250.0); // sum of sums
            assert_eq!(agg.count, 25); // sum of counts
            assert_eq!(agg.last, 20.0); // last of lasts
        }
    }
}
